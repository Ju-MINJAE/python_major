...
# valid_data = np.loadtxt(
#     '/Users/juminjae/Desktop/python/2024_py/머신러닝/valid.txt')
# valid_feat1, valid_feat2, valid_feat3, valid_label = valid_data[:,
...
# def compute_gradients(feat1, feat2, feat3, label, predictions):
#     errors = predictions - label
#     grad_a = np.dot(errors, feat1) / len(feat1)
#     grad_b = np.dot(errors, feat2) / len(feat2)
#     grad_c = np.dot(errors, feat3) / len(feat3)
#     grad_d = np.mean(errors)
#     return grad_a, grad_b, grad_c, grad_d
...
#         feat1, feat2, feat3, label, predictions)
...
#     log_loss = compute_log_loss(label, predictions)
#     log_loss_history.append(log_loss)
#     print(f'Epoch {epoch + 1}/{epochs}, Log Loss: {log_loss:.5f}')
